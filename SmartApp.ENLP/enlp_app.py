...
def __main__():
    myID = register()
    #print("ID:" + str(myID))
    subscribe(myID, {"TAG": TAG_USER_TRANSCRIPT, "text": "$input"}, get_user_emotion) # text to speech
    subscribe(myID, { "NLP_Answer": "$input"}, prepare_answer) # add emotion to answer
    ...
    addFact(myID, TAG_USER_TRANSCRIPT, 1, 100, False, obj_from_erasmus)
