...
import logging
from HAL.thread import HALAudioThread as HALAudio
from HAL.thread import HALVideoThread as HALVideo
...
Log = __setupLogger("HALInteractionLib")


class HALInterface(object):
    def __init__(self, HALAddress="127.0.0.1", HALVideoPort=1234, HALAudioPort=4321, HALControlPort=2314):
        # Config
    ...
    def registerAsVideoReceiver(self, callback):
        ...
        id = self.__getClientID()
        clientThread = HALVideo.HALVideoThread(self.HALAddress, self.HALVideoPort, self.HALControlPort, id, callback)
        if clientThread.connect():
            self.registeredClients[id] = clientThread
            clientThread.start()
            return id
