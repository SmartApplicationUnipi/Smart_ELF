...
public class CrawlingManager {
    ...
    private static final int DEFAULT_MAX_CRAWLING_DEPTH = 2;
    ...
    private void addCrawlingTask(Link link)
    {
        this.executor.submit(() -> {
            ...
            try {
                df = downloadFile(link);
            } catch (IOException e) {
                // In case of Exception, stop immediately
                this.workingCrawlers.decrementAndGet();
                return;
            }

            // Harvest data from the Web
            Crawler c = new HTMLCrawler(df, this.rs);
            ...
            for (Link l : data.getExternalLinks())
                if (l.getDepth() + 1 < DEFAULT_MAX_CRAWLING_DEPTH)
    ...
    public DownloadedFile downloadFile(Link link) throws IOException {
        ...
        return new DownloadedFile(link, sb.toString());
    }
}
